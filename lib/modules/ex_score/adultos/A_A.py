...
def score(data):
    ...
    min_border = -120        # Min threshold for valid point

    base_ang = 0            # Objective angle
    threshold_ang = 20      # Threshold for valid peak
    max_ang = 30            # Threshold for great peak

    n_exp_rep = 15          # Expected repetitions
    n_min_rep = 10          # Minimum reps
    ...
    #Inclinacion al frente

    if(n_min_1<(n_min_rep/2)):
        ang_i_score = 1
        rec = 'Inténtalo de nuevo, debes realizar el movimiento más pronunciado hacia adelante ¡Tú puedes!'
        ...
        ang_i_score = 2
        rec = '¡Vas por muy buen camino! Intenta hacer el movimiento un poco más pronunciado hacia adelante'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(n_max_1<(n_min_rep/2)):
        ang_d_score = 1
        rec = 'Inténtalo de nuevo, debes realizar el movimiento más pronunciado hacia atrás ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_d_score = 2
        rec = '¡Vas por muy buen camino! Intenta hacer el movimiento un poco más pronunciado hacia atrás'
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if((freq_min_1>(0.66*2)) or (freq_max_1>(0.66*2))):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento'
            recommendations.append(rec)
        elif((freq_min_1<(0.66*0.5)) or (freq_max_1<(0.66*0.5)) or n_max_1==0 or n_min_1==0):
