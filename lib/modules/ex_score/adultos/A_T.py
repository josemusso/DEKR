...
def score(data):
    ...
    max_threshold_1 = 150       # Threshold for valid max

    mid_ang = 65                # Threshold for great peak
    min_ang = 80                # Threshold for valid peak

    n_exp_rep = 20              # Expected repetitions
    n_min_rep = 15              # Minimum repetitions
    ...
    if(n_rep<(n_min_rep/2)):
        ang_score = 1
        rec = 'Inténtalo de nuevo, debes realizar el movimiento completo, extendiendo el codo a su máximo ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    elif(med_min_1<=mid_ang):
        ang_score = 3
    else:
        ang_score = 2
        rec = '¡Vas por muy buen camino! No olvides hacer el movimiento completo'
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if(freq_min_1>(0.66*2)):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento.'
            recommendations.append(rec)
        elif(freq_min_1<(0.66*0.5) or n_min_1==0):
