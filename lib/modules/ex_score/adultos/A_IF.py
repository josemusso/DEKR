...
def score(data):
    ...
    threshold_ang = 4       # Threshold for valid peak
    max_ang = 8             # Threshold for great peak

    n_exp_rep = 8           # Expected repetitions
    n_min_rep = 5           # Minimum reps
    ...
    max_window=0.35                                         # Time window (seconds)
    ...
    if(n_min_1<(n_min_rep/2)):
        ang_i_score = 1
        rec = 'Inténtalo de nuevo, debes separar más tus piernas y realizar en movimiento más pronunciado hacia la izquierda ¡Tú puedes!'
        ...
        ang_i_score = 2
        rec = '¡Vas por muy buen camino! Intenta separar un poco más tus piernas y hacer el movimiento un poco más pronunciado hacia la izquierda'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(n_max_1<(n_min_rep/2)):
        ang_d_score = 1
        rec = 'Inténtalo de nuevo, debes separar más tus piernas y realizar en movimiento más pronunciado hacia la derecha ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_d_score = 2
        rec = '¡Vas por muy buen camino! Intenta separar un poco más tus piernas y hacer el movimiento un poco más pronunciado hacia la derecha'
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if((freq_min_1>(0.17*3)) or (freq_max_1>(0.17*3))):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento'
            recommendations.append(rec)
        elif((freq_min_1<(0.17*0.5)) or (freq_max_1<(0.17*0.5)) or n_max_1==0 or n_min_1==0):
