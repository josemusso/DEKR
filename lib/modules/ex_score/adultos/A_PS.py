...
def score(data):
    ...
    min_threshold_1 = 120       # Threshold for valid peak
    min_ang_1 = 150             # Threshold for great peak

    # Knee
    max_border_2 = 250          # Max threshold for valid point
    min_border_2 = 0            # Min threshold for valid point

    min_threshold_2 = 90        # Threshold for valid peak
    min_ang_2 = 150             # Threshold for great peak

    n_exp_rep = 5               # Expected repetitions
    n_min_rep = 4               # Minimum repetitions
    ...
    # Hip   
    if(n_max_1<(n_exp_rep/2)):
        ang_score_1 = 1

    elif(med_max_1>=min_ang_1):
        ang_score_1 = 3
    else:
        ang_score_1 = 2

    # Knee
    if(n_max_2<(n_exp_rep/2)):
        ang_score_2 = 1

    elif(med_max_2>=min_ang_2):
        ang_score_2 = 3
    else:
        ang_score_2 = 2
    ...
    if(ang_score==2):
        rec = '¡Vas por muy buen camino! Recuerda mantener el tronco recto cuando te pares'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    elif(ang_score==1):
        rec = 'Inténtalo de nuevo, debes mantener el tronco recto cuando te pares ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
