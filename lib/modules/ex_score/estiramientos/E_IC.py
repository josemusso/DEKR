...
def score(data):
    ...
    #Arm
    max_border_1 = 300     # Max threshold for valid point
    ...
    if(valid_time<time_thr_min):
        ...
        if(mean_ang_2_o>=mid_thr_2):
            ...
            ang_2_score=2
            rec='¡Muy bien! Intenta inclinarte un poco más, empujando la cadera hacia afuera'
            angle_rec.append(rec)
        else:
            ang_2_score=1
            rec='Inténtalo de nuevo, debes inclinar más tu tronco, empujando la cadera hacia afuera ¡Tú puedes!'
            angle_rec.append(rec)
        
        if(mean_ang_1_o<min_thr_1):
            ang_1_score=1
            rec='¡Falta poco! Busca subir más el brazo para alcanzar la posición ideal ¡Tú puedes!'
            angle_rec.append(rec)
        elif(mean_ang_1_o<mid_thr_1):
            ang_1_score=2
            rec='¡Vas por muy buen camino! Intenta levantar más tu brazo'
            angle_rec.append(rec)
        else:
            ang_1_score=3
        ...
        time_score=3
        rec='¡Fantástico! Superaste las expectativas y completaste todo el tiempo de elongación.'
        time_rec.append(rec)
