...
def score(data):
    ...
    max_border = 500         # Max threshold for valid point
    min_border = -500        # Min threshold for valid point
    ...
    if(n_min_1<(n_min_rep/2)):
        ang_i_score = 1
        rec = 'Inténtalo de nuevo, debes realizar el movimiento más pronunciado hacia la izquierda ¡Tú puedes!'
        ...
        ang_i_score = 2
        rec = '¡Vas por muy buen camino! Intenta hacer el movimiento un poco más pronunciado hacia la izquierda '
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(n_max_1<(n_min_rep/2)):
        ang_d_score = 1
        rec = 'Inténtalo de nuevo, debes realizar el movimiento más pronunciado hacia la derecha ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_d_score = 2
        rec = '¡Vas por muy buen camino! Intenta hacer el movimiento un poco más pronunciado hacia la derecha'
        if(rec not in angle_rec):
            angle_rec.append(rec)
