...
def score(data):
    ...
    max_border = 45         # Max threshold for valid point
    min_border = -45        # Min threshold for valid point

    base_ang = 0            # Objective angle
    threshold_ang = 3       # Threshold for valid peak
    max_ang = 8             # Threshold for great peak

    n_exp_rep = 7           # Expected repetitions
    n_min_rep=6             # Minimum reps
    ...
    max_window=0.7                                          # Time window (seconds)
    ...
    if(n_max_1<(n_min_rep/2)):
        ang_i_score = 1
        rec = 'Inténtalo de nuevo, debes inclinar más tu cabeza hacia la izquierda ¡Tú puedes!'
        ...
        ang_i_score = 2
        rec = '¡Vas por muy buen camino! Intenta inclinar tu cabeza un poco más hacia la izquierda'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(n_min_1<(n_min_rep/2)):
        ang_d_score = 1
        rec = 'Inténtalo de nuevo, debes inclinar más tu cabeza hacia la derecha ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_d_score = 2
        rec = '¡Vas por muy buen camino! Intenta inclinar tu cabeza un poco más hacia la derecha'
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if((freq_min_1>(0.2333*2)) or (freq_max_1>(0.2333*2))):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento'
            recommendations.append(rec)
        elif((freq_min_1<(0.2333*0.5)) or (freq_max_1<(0.2333*0.5)) or n_max_1==0 or n_min_1==0):
