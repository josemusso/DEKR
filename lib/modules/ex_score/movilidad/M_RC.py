...
def score(data):
    ...
    max_border = 150            # Max threshold for valid point
    min_border = 0              # Min threshold for valid point

    max_threshold = 100         # Threshold for valid maxium or minimum

    mid_ang = 35                # Threshold for great peak
    min_ang = 50                # Threshold for valid peak

    n_exp_rep = 10              # Expected repetitions
    n_min_rep = 7               # Minimum repetitions
    ...
    if(n_rep<(n_min_rep/2)):
        ang_score = 1
        rec = 'Inténtalo de nuevo, debes rotar más tu cuerpo ¡Tú puedes!'
        ...
        ang_score = 2
        rec = '¡Vas por muy buen camino! Intenta rotar un poco más tu cuerpo'
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if(freq_min_1>(0.33*2)):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento.'
            recommendations.append(rec)
        elif(freq_min_1<(0.33*0.5) or n_min_1==0):
