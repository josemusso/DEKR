...
def score(data):
    ...
    max_border = 60         # Max threshold for valid point
    min_border = 0        # Min threshold for valid point

    min_ang = 25                    # Threshold for valid max
    mid_ang = 27                    # Threshold for great max

    n_exp_rep = 30                   # Number of desired reps
    n_min_rep = 20                   # Number of minimum reps
    ...
    max_window=0.333                                        # Time window (seconds)
    ...
    if(n_rep<(n_min_rep/2)):
        ang_score = 1
        rec = 'Inténtalo de nuevo, debes estirar más tus brazos ¡Tú puedes!'
        ...
        ang_score = 2
        rec = '¡Vas por muy buen camino! Intenta extender un poco más tus brazos '
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if(freq_max_1>(0.66*3)):
