...
def score(data):
    ...
    max_border = 210        # Max threshold for valid point
    min_border = 0          # Min threshold for valid point

    max_threshold = 140     # Threshold for valid max
    min_ang = 100           # Threshold for valid min
    mid_ang = 80            # Threshold for great min
    
    n_exp_rep = 15          # Number of desired reps
    n_min_rep = 10          # Number of minimum reps
    ...
    if(n_rep<(n_min_rep/2)):
        ang_score = 1
        rec = 'Inténtalo de nuevo, debes flectar más las rodillas ¡Tú puedes!'
        ...
        ang_score = 2
        rec = '¡Vas por muy buen camino! Intenta doblar un poco más tus rodillas '
        if(rec not in angle_rec):
            angle_rec.append(rec)

    if(rep_score>1):
        if(freq_min_1>(0.35*2)):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento.'
            recommendations.append(rec)
        elif(freq_min_1<(0.35*0.5) or n_min_1==0):
