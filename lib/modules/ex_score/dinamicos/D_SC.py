...
def score(data):
    ...
    max_border = 180         # Max threshold for valid point
    min_border = 0        # Min threshold for valid point

    # Knee
    min_ang_1 = 130             # Threshold for valid min
    mid_ang_1 = 120             # Threshold for great min
    max_threshold_1 = 160       # Threshold for valid max

    # Arms
    min_ang_2 = 130             # Threshold for valid min
    mid_ang_2 = 120             # Threshold for valid min
    max_threshold_2 = 160       # Threshold for valid max

    n_exp_rep = 30              # Number of desired reps
    n_min_rep = 25              # Number of minimum reps
    ...
    max_window=0.3                                            # Time window (seconds)
    ...
    df.loc[df['min_1'] >= min_ang_1, 'min_1'] = np.nan
    df.loc[df['max_1'] <= max_threshold_1, 'max_1'] = np.nan
    ...
    # Keep only points on valid threshold.
    df.loc[df['min_2'] >= min_ang_2, 'min_2'] = np.nan
    df.loc[df['max_2'] <= max_threshold_2, 'max_2'] = np.nan
    ...
    #Check if difference between arms and legs it's too big
    if ((n_min_1-(n_min_2/2))>2):
        rec = '¡No olvides mover tus brazos!'
        if(rec not in rep_rec):
            rep_rec.append(rec)
    elif((n_min_1-(n_min_2/2))<-2):
        rec = '¡Recuerda mover las piernas!'
        if(rec not in rep_rec):
            rep_rec.append(rec)
    ...
    if(n_min_1<(n_min_rep/2)):
        ang_score_1 = 1
        rec = 'Inténtalo de nuevo, debes hacer el ejercicio más pronunciado y doblar más las rodillas con cada salto ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    elif(med_min_1<=mid_ang_1):
        ang_score_1 = 3
    else:
        ang_score_1 = 2
        rec = '¡Vas por muy buen camino! Intenta hacer un poco más pronunciado el ejercicio doblando más las rodillas al caer del salto'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    # Brazos    
    if(n_min_2<n_exp_rep):
        ang_score_2 = 1
        rec = 'Inténtalo de nuevo, debes hacer el movimiento más pronunciado moviendo más los brazos ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    elif(med_min_2<=mid_ang_2):
        ang_score_2 = 3
    else:
        ang_score_2 = 2
        rec = '¡Vas por muy buen camino! Intenta mover un poco más tus brazos'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(rep_score>1):
        ...
        elif(freq_min_1<(0.5*0.5) or n_min_1==0):
        ...
        if(freq_min_2>(1*3)):
            ...
            if(rec not in recommendations):
                recommendations.append(rec)
        elif(freq_min_2<(0.5*0.5) or n_min_2==0):
            rec = '¡Vamos, vamos! Tu ritmo es un poco lento, intenta hacer el ejercicio un poco más rápido.'
            if(rec not in recommendations):
                recommendations.append(rec)
