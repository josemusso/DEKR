...
def score(data):
    ...
    min_threshold_1 = 10            # Threshold for valid min
    min_ang = 15                    # Threshold for valid max
    mid_ang = 20                    # Threshold for great max

    n_exp_rep = 8                   # Number of desired reps
    n_min_rep = 6                   # Number of minimum reps
    ...
    if(n_rep<(n_min_rep/2)):
        ang_score = 1
        rec = 'Inténtalo de nuevo, debes separar más tus piernas y bajar lo máximo posible ¡Tú puedes!'
        ...
        ang_score = 2
        rec = '¡Vas por muy buen camino! Intenta separar un poco más tus piernas y bajar más'
        if(rec not in angle_rec):
            angle_rec.append(rec)
