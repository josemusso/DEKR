...
def score(data):
    ...
    max_border = 200            # Max threshold for valid point
    min_border = 0              # Min threshold for valid point

    # Knee
    min_ang_1 = 120             # Threshold for valid min
    mid_ang_1 = 110             # Threshold for great min
    max_threshold_1 = 160       # Threshold for valid max

    # Hip
    min_ang_2 = 120             # Threshold for valid min
    mid_ang_2 = 110             # Threshold for valid min
    max_threshold_2 = 160       # Threshold for valid max

    n_exp_rep = 18              # Number of desired reps
    n_min_rep = 15              # Number of minimum reps
    ...
    #Check if difference between hip and knee it's too big
    if ((n_min_1-n_min_2)>2):
        rec = '¡Recuerda levantar las piernas!'
        if(rec not in rep_rec):
            rep_rec.append(rec)
    elif((n_min_1-n_min_2)<-2):
        rec = '¡No olvidar doblar las rodillas!'
        if(rec not in rep_rec):
            rep_rec.append(rec)
    ...
    if(n_min_1<(n_min_rep/2)):
        ang_score_1 = 1
        rec = 'Inténtalo de nuevo, debes doblar más tu rodilla al levantar la pierna ¡Tú puedes!'
        ...
        ang_score_1 = 2
        rec = '¡Vas por muy buen camino! Intenta doblar un poco más tu rodilla cuando levantes la pierna '
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(n_min_2<(n_min_rep/2)):
        ang_score_2 = 1
        rec = 'Inténtalo de nuevo, debes levantar más tus piernas ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_score_2 = 2
        rec = '¡Vas por muy buen camino! Intenta elevar un poco más tus piernas'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(rep_score>1):
        ...
        elif(freq_min_1<(0.4*0.5) or n_min_1==0):
        ...
        if(freq_min_2>(0.4*2)):
            ...
            if(rec not in recommendations):
                recommendations.append(rec)
        elif(freq_min_2<(0.4*0.5) or n_min_2==0):
            rec = '¡Vamos, vamos! Tu ritmo es un poco lento, intenta hacer el ejercicio un poco más rápido.'
            if(rec not in recommendations):
                recommendations.append(rec)
