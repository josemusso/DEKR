...
def score(data):
    ...
    min_threshold_1 = 90        # Threshold for valid min
    min_ang_1 = 120             # Threshold for valid max
    mid_ang_1 = 140             # Threshold for great max

    min_threshold_2 = 10        # Threshold for valid min
    min_ang_2 = 10              # Threshold for valid max
    mid_ang_2 = 15              # Threshold for great max

    n_exp_rep = 12              # Number of desired reps
    ...
    if ((n_max_1-n_max_2)>2):
        ...
        rec = '¡No olvides elevar los brazos!'
        if(rec not in rep_rec):
            rep_rec.append(rec)
    ...
    if(n_max_2<(n_min_rep/2)):
        ang_score_2 = 1
        rec = 'Inténtalo de nuevo, debes mover más tus piernas ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_score_2 = 2
        rec = '¡Vas por muy buen camino! Intenta mover un poco más tus piernas'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(rep_score>1):
        ...
        elif(freq_max_1<(0.5*0.5) or n_max_1==0):
        ...
        if(freq_max_2>(0.5*3)):
            ...
            if(rec not in recommendations):
                recommendations.append(rec)
        elif(freq_max_2<(0.5*0.5) or n_max_2==0):
            rec = '¡Vamos, vamos! Tu ritmo es un poco lento, intenta hacer el ejercicio un poco más rápido.'
            if(rec not in recommendations):
                recommendations.append(rec)
