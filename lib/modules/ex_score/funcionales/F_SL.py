...
def score(data):
    score=True
    ## Calification rules
    max_border_1 = 220         # Max threshold for valid point
    min_border_1 = 0        # Min threshold for valid point

    max_border_2 = 220         # Max threshold for valid point
    min_border_2 = 000        # Min threshold for valid point

    # Knee
    min_ang_1 = 140             # Threshold for valid min
    mid_ang_1 = 120             # Threshold for great min
    max_threshold_1 = 160       # Threshold for valid max

    # Hip
    min_ang_2 = 140             # Threshold for valid min
    mid_ang_2 = 120             # Threshold for great min
    max_threshold_2 = 160       # Threshold for valid max

    n_exp_rep = 10              # Number of desired reps
    n_min_rep = 7              # Number of minimum reps
    ...
    if(n_min_1<(n_min_rep/2)):
        ang_score_1 = 1
        rec = 'Inténtalo de nuevo, debes doblar más tus rodillas ¡Tú puedes!'
        ...
        ang_score_1 = 2
        rec = '¡Vas por muy buen camino! Intenta flectar un poco más tus rodillas'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(n_min_2<(n_min_rep/2)):
        ang_score_2 = 1
        rec = 'Inténtalo de nuevo, debes inclinar más tu cuerpo mientras doblas tus rodillas ¡Tú puedes!'
        if(rec not in angle_rec):
            angle_rec.append(rec)
        ...
        ang_score_2 = 2
        rec = '¡Vas por muy buen camino! Intenta inclinar un poco más tu cuerpo cuando doblas las rodillas'
        if(rec not in angle_rec):
            angle_rec.append(rec)
    ...
    if(rep_score>1):
        ...
        if(freq_min_2>(0.33*2)):
            rec = '¡Ups, parece que vas muy rápido! Intenta hacer el ejercicio un poco más lento.'
            if(rec not in recommendations):
                recommendations.append(rec)
        elif(freq_min_2<(0.33*0.5) or n_min_2==0):
            rec = '¡Vamos, vamos! Tu ritmo es un poco lento, intenta hacer el ejercicio un poco más rápido.'
            if(rec not in recommendations):
                recommendations.append(rec)
